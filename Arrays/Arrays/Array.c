...
/*
	...
	return	返回元素下标
*/
int indexLastIndexOf(PList pdst, void * pel)
{
	if (NULL == pdst)
	{
		printf("这个数组不能为空");
...
*/
bool delete(PList pdst, void * pel, int(*pfind)(PList, void *, int(*compre)(void *pa, void *pb)))
{
	int flag = compre((char *)pdst->buffer + pdst->elemsize, pel);
	if (flag==-1)
	{
		printf("没有找到这个数,无法删除");
	} 
	...
	{
		//从前开始,让其之后的数往前移动
		for (int i = flag; i < pdst->length;i++)//其中flag是找到元素的下标
		{
			memcmp((char *)pdst->buffer + i*pdst->elemsize, (char *)pdst->buffer + (i + 1)*pdst->elemsize, pdst->elemsize);
		}
		pdst->length -= 1;
